...
void OLED_W_SCL(uint8_t BitValue)
{
    if(BitValue)
    {
        DL_GPIO_setPins(OLED_CL_PORT, OLED_CL_PIN);
    }
    else
    {
        DL_GPIO_clearPins(OLED_CL_PORT, OLED_CL_PIN);
    }
    /* I2C标准模式需要至少4.7μs的时钟周期 */
    delay_cycles(2);
}
...
void OLED_W_SDA(uint8_t BitValue)
{
    if(BitValue)
    {
        DL_GPIO_setPins(OLED_DA_PORT, OLED_DA_PIN);
    }
    else
    {
        DL_GPIO_clearPins(OLED_DA_PORT, OLED_DA_PIN);
    }
    /* SDA变化后需要保持稳定时间 */
        delay_cycles(2);
