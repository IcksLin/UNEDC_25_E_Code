...
void tracking_mode(PID* tracking){
    ...
    if(ABS(onto)<=44){
    PID_Control(&pid_r, 90+0.7*onto, current_speed.speed_R);
    PID_Control(&pid_l, 90-0.7*onto, current_speed.speed_L);
    PID_Control(tracking , 0, -onto);

    }else {
        PID_Control(&pid_r, 50+0.4*onto, current_speed.speed_R);
        PID_Control(&pid_l, 50-0.4*onto, current_speed.speed_L);
        PID_Control(tracking , 0, -onto);
