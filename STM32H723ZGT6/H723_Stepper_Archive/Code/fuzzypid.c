...
float fuzzy_pid_control(FuzzyPID* fuzzy_pid, float target, float current_value) {
    ...
    if (fuzzy_pid->use_fuzzy) {
        ...
        fuzzy_pid->Kp = LIMIT(fuzzy_pid->Kp, 0.50f, 3.0f);
        fuzzy_pid->Kd = LIMIT(fuzzy_pid->Kd, 0.0f, 1.3f);
        fuzzy_pid->Ki = fuzzy_pid->Kp / fuzzy_pid->Ti;
    }
...
//模糊PID控制器初始化函数(10sm控制周期)
void init_all_pid_new(void) {
    const float dt = 0.01f; 
   //速度环
    fuzzy_pid_init(&pid_x, 0.730f, dt, 
                   0.02,                              // Kp, Ti, Kd
                   5.0f,                           // error_filter
                   300.0f, -300.0f,                // output_max, output_min
                   20000.0f, -20000.0f,                // integral_max, integral_min
                   0);                             // use_fuzzy

    fuzzy_pid_init(&pid_y, 0.730f, dt, 0.02,
                   5.0f,
                   300.0f, -300.0f,
                   20000.0f, -20000.0f,
                   0);
...
void set_pid_index(FuzzyPID* fuzzy_pid,float kp,float kd,float ki_change,float dt) {
    fuzzy_pid->Kp = kp;
    fuzzy_pid->Kd = kd;
    fuzzy_pid->Ki = ki_change;
}
