...
void menu_mode_1(uint8_t cl_action)
{
    ...
    case 5: // 长按改变方向
        ...
        while (rect.x == 999 && rect.y == 999 && finding_flag)
        {
            ...
            if (mode2_current_time - mode2_start_time >= 3500)
            {
                HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET); // Set the laser pin to high
            }
...
void menu_mode_2(uint8_t cl_action)
{
    ...
    if (mode3_start_flag && rect.x != 999 && rect.y != 999)
    {
        ...
        move_distance.y = rect.x - target_point.x + (int32_t)(Yaw_a / 2);
    }
    else
    {
        move_distance.x = 0;
        move_distance.y = (int32_t)(Yaw_a / 2);
    }
