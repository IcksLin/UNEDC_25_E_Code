...
bool ParsePacket(const char *packet)
{
    ...
    else if (strncmp(packet, "@GREEN:", 7) == 0)
    {
        // 解析GREEN的1个坐标
    ...
    else if (strncmp(packet, "@RECT_CENTER:", 13) == 0)
    {
        // 解析RECT的1个坐标
        return (sscanf(packet + 13, "(%d,%d)", &rect.x, &rect.y) == 2);
    }
    else if (strncmp(packet, "@LASER:", 7) == 0)
    {
        // 解析LASER的1个坐标
        return (sscanf(packet + 7, "(%d,%d)", &target_point.x, &target_point.y) == 2);
    }
