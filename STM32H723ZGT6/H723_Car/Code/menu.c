...
void menu_mode_0(uint8_t cl_action)
{
    ...
    if (start_flag == 1 && (current_time - start_time) >= 1100 && (current_time - start_time) <= 3600)
    {
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET); // Set the start pin to high
    }
    else
    {
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_RESET); // Set the start pin to low
    }
...
void menu_mode_1(uint8_t cl_action)
{
    ...
    {
        // 时间一到，便直接开启激光
        if (mode2_current_time - mode2_start_time >= 3000 && mode2_current_time - mode2_start_time <= 7000)
        {
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET); // Set the start pin to high
        }
        else
    ...
    case 0:
        ...
        {
            // 步进巡视，知道xy反了，别骂了别骂了
            CalculateStepperSpeed(&StepperMotor_y, -SCANF_SPEED); // 恒定SCANF_SPEED脉冲横向巡视
            // 处理数据包，更新坐标
    ...
    case 5: // 长按改变方向
        ...
        {
            // 步进巡视，知道xy反了，别骂了别骂了
            CalculateStepperSpeed(&StepperMotor_y, SCANF_SPEED); // 恒定SCANF_SPEED脉冲巡视
            // 处理数据包，更新坐标
            if (UART_GetPacket(&received_packet))
            {
                // 处理完整数据包（如转发或解析
...
{
    // 1 start, 0 stop
    static uint8_t mode3_start_flag = 0;

    switch (cl_action)
    {
    case 0:
        mode3_start_flag = 1; // enable the flag
    ...
    /****************visualization(start)***************/
    if (mode3_start_flag && rect.x != 999 && rect.y != 999)
    {
        // stare at the target by IMU
        if (ABS(Yaw_a) > 300)
        {
            move_distance.x = rect.y - target_point.y; // IMU
            move_distance.y = rect.x - target_point.x + (int32_t)(Yaw_a);
        }
        else
        {
            move_distance.x = rect.y - target_point.y; // IMU
    ...
    else
    {
        ...
        {
            move_distance.x = 0;
            move_distance.y = (int32_t)(Yaw_a / 2.5);
        }
        else
        {
            move_distance.x = 0;
    ...
    if (mode3_start_flag == 1)
    {
        OLED_ShowString(0, 0, "Start", OLED_8X16);
        OLED_ShowString(0, 1 * 16, "X:", OLED_8X16);
        OLED_ShowFloatNum(0, 1 * 16, Yaw_a, 3, 2, OLED_8X16);
        OLED_ShowFloatNum(4, 2 * 16, move_distance.x, 4, 1, OLED_8X16);
    }
    else
    {
